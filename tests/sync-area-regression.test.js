...
test('AMR does not return home immediately when no pickup is available after releasing', () => {
  const context = loadAppContext();
  ...
  const result = vm.runInContext(`(() => {
    ...
    const pickup = graphManager.addNode('pickup', 100, 200, 'Pickup');
    const relay = graphManager.addNode('waypoint', 220, 200, 'Relay');
    const dropoff = graphManager.addNode('dropoff', 340, 200, 'Drop');
    graphManager.addLoopSegment(pickup.id, relay.id);
    graphManager.addLoopSegment(relay.id, pickup.id);
    graphManager.addLoopSegment(relay.id, dropoff.id);
    graphManager.addLoopSegment(dropoff.id, relay.id);

    const home = graphManager.addNode('home', 220, 120, 'Home');
    graphManager.addHomeSegment(relay.id, home.id);

    const amr = simulationLogic.createInitialAmr(dropoff, 0);
    amr.id = 'amr_1';
    amr.state = 'releasing';
    amr.currentNodeId = dropoff.id;
    amr.currentPath = null;
    amr.pathIndex = 0;
    amr.waitTimer = state.config.releaseTime;
    state.amrs = [amr];

    state.config.releaseTime = 0;
    state.config.pickupSupplyInterval = 9999;
    state.pickupSupply.set(pickup.id, { availableCarts: 0, nextSupplyTime: 9999 });

    simEngine.update(0.1);

    return {
      state: state.amrs[0].state,
      currentNodeId: state.amrs[0].currentNodeId
    };
  })()`, context);

  assert.equal(result.state, 'waiting_pickup');
});
